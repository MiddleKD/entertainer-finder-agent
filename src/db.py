...
class VectorDBClient:
    ...
    def insert(self, point_id: int, vector: List[float], payload: Optional[Dict[str, Any]] = None) -> bool:
        """
        단일 포인트를 벡터 데이터베이스에 삽입합니다.
    ...
    def insert_bulk(self, points: List[Dict[str, Any]]) -> bool:
        """
        여러 포인트를 벡터 데이터베이스에 일괄 삽입합니다.
        
        Args:
            points: 삽입할 포인트 리스트 (각 포인트는 id, vector, payload를 포함)

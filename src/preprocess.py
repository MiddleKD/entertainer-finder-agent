...
class Preprocessor:
    ...
    def clean_na(self):
        self.processed_df = self.processed_df.dropna(axis=1, how="all").where(pd.notna(self.processed_df), None)
        return self.processed_df

    def get_profile_image_list(self):
        profile_dict = self.processed_df[[col for col in self.processed_df if "pic" in col]].T.to_dict()
        profile_image_list = [value for cur in profile_dict.values() for value in cur.values() if pd.notna(value)]
        ...
        for _, row in self.processed_df.iterrows():
            data = {
                ...
                "payload": {
                    ...
                    "images": [row[cur] for cur in row.keys() if "_pic" in cur if row[cur] is not None],
                },
                "vector":
                    {
                        "face": None,
            ...
            prompt_source = {
                ...
                "job": meta["job"],
                "specialty": validate(
                    " ".join(str(meta[key]) for key in ["specialty", "specialty_etc",] if meta.get(key))
                ),
                "tags": meta["keyword"],
                "field": validate(
                    " ".join(str(meta[key]) for key in ["field1", "field2", "field3", "field4"] if meta.get(key))
                ),
                "experience": validate(
                    " ".join(str(meta[key]) for key in ["last_work", "last_adver"] if meta.get(key))
                ),
...
if __name__=="__main__":
    ...
    for data in tqdm(processed_data_list):
        ...
        try:
            face_embeddings = face_model.run_model(profile_images, only_return_face = True)
        except Exception as e:
